...
class ConvertThread(QThread):
    ...
    def run(self):
        logger = QtProgressLogger(lambda progress: self.progress_signal.emit(progress))
        ...
        try:
            for index,video_path in enumerate(self.video_paths):
                ...
                # Convert audio
                video.write_audiofile(audio_file, bitrate=f"{self.bitrate}k",logger = logger)

                self.remaining_files_signal.emit(f"completed {index+1} of {len(self.video_paths)}")
...
class VideoToAudioApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.status = QLabel("status: completed 0 of 0",self)
    ...
    def start_conversion(self):
        ...
        self.convert_thread.remaining_files_signal.connect(self.status_label)
